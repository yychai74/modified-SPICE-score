...
//			System.out.println(can_sg.toReadableString());
		...
		String s = "a tennis racket with 3 tennis balls resting on it being held by a tennis player";
		List<SceneGraph> t = (List<SceneGraph>) parser.parseCaptions(
				Collections.singletonList(s)
				);
		System.out.println(t.get(0).toReadableString());

//		try {
//			File file = new File("test_out.txt");
//			PrintStream ps = new PrintStream(new FileOutputStream(file));
//			ps.println(s);
//			ps.println(t.get(0).toReadableString());// 往文件里写入字符串
